...
async def read_user_by_id(user_id: int, session: Annotated[Session, Depends(get_session)]):
...
async def read_user_by_email(email: str, session: Annotated[Session, Depends(get_session)]):
...
async def read_all_users(session: Annotated[Session, Depends(get_session)]):
    users = await get_all_users(session)
    return users


async def update_user_by_id(user_id: int, user_update: UserUpdate, session: Annotated[Session, Depends(get_session)], producer: KAFKA_PRODUCER = KAFKA_PRODUCER):
    ...
    event_dict = {"event_type": "user_updation",
                  "user_id": user_id, **updated_user_dict}
...
async def add_payment_details(user_id: int,
    ...
    db_payment = PaymentDetails(
        user_id=user_id, **payment_details.model_dump())
    session.add(db_payment)
    session.commit()
    session.refresh(db_payment)
    user_name = await get_user_name_from_id(user_id)
    payment_details_dict = db_payment.model_dump()
    event_dict = {"event_type": "user_payment_details_added",
                  "user_name": user_name, **payment_details_dict}
    await produce_message("user_payment_details_added", event_dict, producer)

    return db_payment


async def get_payment_details(user_id: int, session: Annotated[Session, Depends(get_session)]):
    db_statement = select(PaymentDetails).where(
        PaymentDetails.user_id == user_id)
    payment_details = session.exec(db_statement).first()
    if not payment_details:
        raise HTTPException(
            status_code=404, detail="Payment details not found")
